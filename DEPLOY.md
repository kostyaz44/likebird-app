...
## 🔥 Firebase Realtime Sync — Настройка

Начиная с этой версии все устройства синхронизируются через Firebase в реальном времени.

### Что уже сделано в коде:
- ✅ `src/firebase.js` — конфиг и хелперы
- ✅ `src/LikeBirdApp.jsx` — функция `save()` пишет в Firebase, подписки обновляют всех
- ✅ `package.json` — добавлена зависимость `firebase ^10`

### После получения файлов выполните:

```bash
# 1. Установите зависимости (firebase добавлен)
npm install

# 2. Соберите проект
npm run build

# 3. Запушьте на GitHub — Vercel пересоберётся автоматически
git add .
git commit -m "Add Firebase realtime sync"
git push
```

### Настройка Firebase Console (важно!):

1. Откройте [console.firebase.google.com](https://console.firebase.google.com) → ваш проект
2. **Realtime Database → Rules** — установите правила:
...
3. Нажмите **Publish** — готово!

### Как работает синхронизация:

| Действие | Результат |
|----------|-----------|
| Сотрудник добавляет отчёт | Все устройства получают его в течение ~1 сек |
| Изменение склада | Моментально обновляется у всех |
| Чат, расписание, бонусы | Realtime для всех |
| Вход/сессия, имя сотрудника | Только локально (не синхронизируются) |

### Синхронизируемые данные (28 ключей):
Отчёты, расходы, склад, выдачи, зарплаты, карты, партнёры, расписание, мануалы, настройки, сотрудники, план продаж, аудит, товары, локации, себестоимость, штрафы, бонусы, отпуска, рейтинги, чат, история склада, списания, автозаказ, KPI.
